...
def update_last_chunk(project_id):
    ...
    if last_chunk is None:
        return jsonify({"error": "No last_chunk provided"}), 400
    
    try:
        if manager.update_last_chunk(project_id, int(last_chunk)):
            return jsonify({"status": "updated", "project_id": project_id, "last_chunk": last_chunk})
        else:
            return jsonify({"error": "Project not found"}), 404
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_project_metadata(project_id):
    ...
    # Fallback si no hay metadata consolidada pero sí chunks (raro pero posible)
    return jsonify({"error": "Consolidated metadata not found"}), 404
...
def download_project_audio(project_id):
    ...
    if os.path.exists(status_path):
        ...
        # Robustez: Aceptar si el flag está activo O si todos los chunks han sido intentados
        # (algunos pueden haber terminado en error, pero queremos permitir la descarga de lo que hay)
        total = status.get("total_chunks", 999999)
        completed = status.get("completed_chunks", 0)
        errors = sum(1 for ch in status["chunks"] if ch["status"] == "error")
        
        if status.get("is_finished") or (completed + errors >= total):
            try:
...
def speak():
    ...
    try:
        # Usar el generador seguro que maneja sub-chunking y limpieza
        _, samples, sample_rate = manager._generate_audio_safe(text, voice, speed, lang)
